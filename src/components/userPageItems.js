     ...
     axios
        ...
        .then((res) => {
          ...
          if (!res.data.success && !res.data.dialog) {
            console.log("dialog "+res.data.uid);
            setDeleteAndUpdateCart({
              message: res.data.message,
              uid: res.data.uid,
              id: res.data.id,
            });
            console.log(deleteAndUpdateCart)
            handleClickOpen();
...
<Card className={classes.root}>
      ...
      <CardContent>
        ...
        <Grid container>
                  ...
                  <Grid item xs>
                    <div>
                      <Button onClick={() => incBtn(restaurant._id)}><AddIcon/></Button>
                      { countItems1.length > 0 ? countItems1.map((item, key) => (  
                        item._id == restaurant._id ?
                        <div key={item._id} style={{fontWeight: "bold"}}>
                       {item.count} 
                       </div> : ""
                       )) : 0}
                      <Button onClick={() => decBtn(restaurant._id)}><RemoveIcon/></Button>
                    </div>
                  </Grid>
                  </Grid>
      </CardContent>
